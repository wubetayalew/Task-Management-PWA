   ...
   <div class="centered-div">
    <div class="card">
       ...
       <div class="card-actions">
        <form action="{{route('task.deletePermanently',['task'=>$task])}}" method="post">
          @csrf
          @method("delete")
             <input type="submit" value="Delete Permanently" style="background-color: red; color: white; padding: 10px; border: 2px solid red;
                border-radius: 5px;">
          </form>
          <form action="{{route('task.recover',['task'=>$task])}}"  method="post">
            @csrf
            @method('put')
             <input type="submit" value="Recover" style="background-color: #3498db; color: white; padding: 10px; border: 2px solid #3498db;
